...
if demo_data_radio == 'Yes' or uploaded_file is not None:
    ...
    if widget == "Anomalies check":
        ...
        dis_fit = [[result_norm[1], result_exp[1], result_lognorm[1], result_weibull2[1]], 
                   [str(result_norm[1] > 0.05), str(result_exp[1] > 0.05), str(result_lognorm[1] > 0.05), str(result_weibull2[1] > 0.05)]]
        st.dataframe(pd.DataFrame(dis_fit, columns = ['Normal', 'Exponential', 'Log-Norm', 'Weibul'], index = ['P-value', 'P > t']))
        ...
        if out_cou == 'All ids': 
            ...
            st.write(df_AllOut[df_AllOut[out_id_col].startswith(out_cou)])
